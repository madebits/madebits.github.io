...
##Side Note: Windows Where Art Thou

Windows operating system is nowadays a shadow of the former self. The decline come even before Window 8. Windows 8 was the effort to reverse that trend. Windows 10 is the effort to reverse some of the Windows 8 failure. It remains to be seen, how that could work. Windows is not bad and it is getting better every day, even when it comes to collecting more user data. However, in almost all directions, people are moving away from it.

Software started as small applications for self-usage was for a long time in the past a major force behind diversity and multiple choice of Windows platform applications. Starting with Windows 8, Microsoft lost a lot of private power users, that now write applications and scripts for other operating systems and have no more any hobbyist motivation to write software for Windows. With Windows store creation, the old open Windows platform for Windows software is unattractive for hobbyist, and writing applications for the windows store is not really worth it for the professionals outside the enterprise world. 

Power users and software professionals rely nowadays mostly on other platforms for software development. The software development tools on Windows are still quite good and in some directions still to be matched. Tools from other competitors, such as [JetBrains](https://www.jetbrains.com/) are filling the gap slowly across all other platforms, whereas most of good Windows development tools are still Windows only. That was not a problem when Windows ruled the source code world, but today Visual Studio is not as important as it is used to be (even for writing Linux code). With .NET coming to other platforms, Microsoft acknowledges some of that.

The majority of remaining users definitively prefer Android (even as desktop replacement) or iPhone to Windows. There is a steady user base still remaining in Windows, as there is still a lot of existing software solutions that are Windows only and Microsoft software offerings cover a wide range of business domains. It is a matter of time until that software either moves to cloud services, or to other operating systems.
