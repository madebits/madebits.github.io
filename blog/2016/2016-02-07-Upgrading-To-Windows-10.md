...
I am not sure why Microsoft is insisting on such intrusive data collection strategies. Data collection makes sense to counterweight Google and most people will accept that at some point of time. On the other side, insisting on collecting data at any cost and not offering some way to disable it fully, pushes existing power users out of Windows. Hobbyist power user created software stood in the past for a large part of Windows software diversity offerings. With Windows store and invasive data collection power users generally avoid Windows, resulting in less useful new software being written for that platform.
