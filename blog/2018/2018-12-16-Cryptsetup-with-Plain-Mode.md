...
Related to `--offset` and `--size` is `--skip` that tells cryptsetup first sector number to use for IV calculation (by default `0` is used in all parts). I am not sure why that info is exposed to the user, very likely to enable using a different *IV seed* for each part.
