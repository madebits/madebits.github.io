...
1. Disable recently use files list (has to be done for both current user and `root`):
	...
	If using GThumb, apply same to `$HOME/.config/gthumb/history.xbel` (same for the `root` user) and remove catalogs plugin. If you use GNOME, apply same to `$HOME/.local/share/gnome-shell/application_state` and to `chmod a=-w $HOME/.config/nautilus/desktop-metadata`.
...
1. Setting up a protected encrypted folder in [Ubuntu](https://help.ubuntu.com/community/EncryptedPrivateDirectory) is done via:
	...
	To move `bash` history to the private folder add in `HOME/.bashrc`:

	```
	export HISTFILE=/home/$USER/Private/.bash_history
	```
