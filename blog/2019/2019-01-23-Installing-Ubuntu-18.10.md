...
I think with minor tweaks, Ubuntu 18.10 UI is usable out of the box, without having to install some other desktop variant. The overall UI direction looks ok, I can imagine using next LTS release UI as default desktop. Thought, I expect to see more crap like `snap` and `ubuntu-report` being added there by default.
