...
**[GRunner](https://www.npmjs.com/package/grunner)** is a task runner for [Node.js](https://nodejs.org/), inspired by and compatible with [Gulp](http://gulpjs.com/). Most [Gulp](https://github.com/gulpjs/gulp/tree/4.0) plugins and Gulp streams, such as, `gulp.src`, can be used as they are with GRunner. GRunner task scheduler is based on [async](https://github.com/caolan/async) and it is *subjectively* better :). If you ever found Gulp task handling somehow limiting, you are at the right place. GRunner uses some [ES2015](http://www.ecma-international.org/ecma-262/6.0/) features, so it needs a recent version of Node.js. GRunner is useful if you enjoy automating continuous building, integration and delivery of software by writing Node.js code.
...
* `--C` - GRunner does circular dependency loop detection as it runs the tasks by default. If you ever do **not** need that functionality, turn it *off* by specifying this option.
...
When you define a task (via `G.t`) you can pass optional task dependencies to be run before it as second argument. Dependencies can be a task name as string, or a array of task name strings. *Nested* arrays are supported and have special meaning. Examples:
...
GRunner tasks are non-blocking, but there is no direct *parallelism* involved. If you invoke external tools in the tasks, they will run in separate processes and achieve thus parallelism. Arbitrary Node.js code can be run similarly in a different process, if wished, by using `node` as an external tool (you cannot easy share process memory, but build and deploy tasks usually share disk files). As an example, lets use [GSpawn](https://www.npmjs.com/package/gspawn) library to run some arbitrary JS code found in an external file, in a separate process as part of a GRunner task function:
...
Some simple low level file manipulation function wrappers are provided. You can either use these (can be called on any GRunner instance), or `gulp.src` / `dest`, or other similar libraries.
...
* `g.files(dir, [recursive], [filter])` - normally you would use `gulp.src` to get a stream of files, or the `glob` module. I have found `glob` module to be not very reliable with [UNC](https://en.wikipedia.org/wiki/Universal_Naming_Convention) paths, so `g.files` is provided as a simple alternative. `g.files` returns all files in a given folder specified by `dir` as an object stream. Details:
 
  * `dir` - a folder (string), or an array of folders to scan for files.
  * If `recursive` is `true` then all files in all sub-folders of `dir` are returned. 
  * `filter` is an optional function of form `fn(fileObject)` that can be used to filter which files are included by returning `true` (or `false` not to include them). The `fileObject` is of form: `{dir, path, relative, name, ext, stats}` - where `dir` is the path passed to `g.files`, `path` is the absolute path of file, `relative` is the relative file path from `dir`, `name` is the filename with extension, `ext` is the file extension together with the dot, and `stats` is the `fs.Stats` object.
  * `g.files` returns *immediately* a `stream` of `fileObject`s. There is no warranty on the order of files returned. Assuming you have imported `gulp` `File` class, you can interface the file data provided by the `g.files` stream to the `gulp` one, by using code similar to the following:

    ```javascript
    g.t('tt', () => {
      return g.files(['./app', './src'], true, o => (/^\.js$/i).test(o.ext))
        .pipe(g.pipeThrough((o, _cb) => {
          _cb(null, new File({ base: '', cwd: '', path: o.path }));
        }))
        .pipe(gulp.dest(...));  
    });
  ```
